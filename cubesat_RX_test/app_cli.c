 ...
 *
 * [변경 내역]
 *   - cli_set_device_id(): "set_id <1-4>" — NVM3에 device_id 영구 저장
 *     최초 1회 실행 후 재부팅하면 부팅 시 자동 로드됨
 *******************************************************************************
...
// ─── [신규] app_init.c와 동일한 NVM3 키 ──────────────────────────────────────
#define DEVICE_ID_NVM3_KEY  ((nvm3_ObjectKey_t)0x00001)
...
// ─── [신규] app_init.c에서 NVM3 로드된 device_id 참조 ────────────────────────
...
/******************************************************************************
 * [신규] CLI - set_id command
 *
 * 사용법: set_id <device_id>
 *   device_id: 1~4
 *
 * 동작:
 *   NVM3에 device_id를 영구 저장 후 재부팅 안내.
 *   재부팅 후 app_init.c에서 자동 로드되며,
 *   NETWORK_UP 시 TX에 ID 알림(0xA0) 메시지를 전송하여 slave_table에 등록됨.
 *
 * 최초 1회만 실행하면 됨. 이후 전원이 꺼져도 NVM3에 유지됨.
 *
 * 예시:
 *   > set_id 1    ← 이 RX의 ID를 1번으로 설정
 *   > reset       ← 재부팅 (set_id 후 반드시 필요)
 *****************************************************************************/
void cli_set_device_id(sl_cli_command_arg_t *arguments)
{
  ...
  if (new_id < 1 || new_id > 4) {
    app_log_error("set_id: invalid value=%d (valid range: 1~4)\n", new_id);
    return;
  }

  Ecode_t ecode = nvm3_writeData(nvm3_defaultHandle,
                                 DEVICE_ID_NVM3_KEY,
                                 &new_id,
                                 sizeof(new_id));

  if (ecode == ECODE_NVM3_OK) {
    my_device_id = new_id;  // 런타임 변수도 즉시 갱신
    app_log_info("Device ID set to %d and saved to NVM3.\n", new_id);
    app_log_info("Please reboot (CLI: reset) to apply.\n");
  } else {
    app_log_error("NVM3 write FAILED: 0x%lX\n", ecode);
  }
}
...
void cli_join(sl_cli_command_arg_t *arguments)
{
  ...
  if (check_channel(channel) == false) return;
